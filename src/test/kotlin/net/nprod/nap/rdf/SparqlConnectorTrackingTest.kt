...
class SparqlConnectorTrackingTest {
    ...
    fun `test query tracking when enabled`() {
        ...
        SparqlConnector.trackQuery(testQuery, 100L)
    ...
    fun `test query tracking when not enabled`() {
        ...
        val testQuery = "SELECT * WHERE { ?s ?p ?o } LIMIT 10"
        SparqlConnector.trackQuery(testQuery, 50L)
    ...
    fun `test multiple queries tracking`() {
        ...
        queries.forEachIndexed { index, query ->
            SparqlConnector.trackQuery(query, (index + 1) * 50L)
        }
    ...
    fun `test clear tracking`() {
        ...
        // Track a query
        val testQuery = "SELECT * WHERE { ?s ?p ?o } LIMIT 10"
        SparqlConnector.trackQuery(testQuery, 75L)
    ...
    fun `test queries are filtered when logQuery is false`() {
        ...
        SparqlConnector.trackQuery("SELECT * WHERE { ?s ?p ?o }", 100L, true)

        // Queries with logQuery = false - should NOT be tracked
        SparqlConnector.trackQuery("SELECT * FROM cache WHERE { ?s ?p ?o }", 50L, false)
        SparqlConnector.trackQuery("SELECT ?predicate ?node WHERE { <uri> ?predicate ?node }", 30L, false)

        // Another query with logQuery = true - should be tracked
        SparqlConnector.trackQuery("CONSTRUCT { ?s ?p ?o } WHERE { ?s ?p ?o }", 200L, true)
